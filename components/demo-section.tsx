 ...
 * redesign (ticket 6.5's content guard: anonymized example texts stay as-is) and through 6.6's
 * bilingual landing (ticket 6.6, part E: "the two review examples stay Polish with an 'example
 * in Polish' note", rather than translated). Restaurant name AND address are redacted — the
 * ticket only asked for the name, but a real Warsaw street address next to a very specific
 * review is itself enough to re-identify the place on Google, so it's blanked too (disclosed,
 * carried over from 4.1).
 *
...
const COPY = {
  pl: {
    eyebrow: "Zobacz sam",
    h2: "Prawdziwe opinie. Prawdziwe odpowiedzi.",
    note: "Przykładowe opinie i odpowiedzi przez ReviewGuide.",
    place: "Restauracja",
    district: "Śródmieście",
    replyTag: "Odpowiedź ReviewGuide",
    exampleLanguageNote: null,
  },
  en: {
    eyebrow: "See for yourself",
    h2: "Real reviews. Real responses.",
    note: "Example reviews and responses from ReviewGuide.",
    place: "Restaurant",
    district: "City centre",
    replyTag: "ReviewGuide response",
    exampleLanguageNote: "(example in Polish)",
  },
} as const;
...
export function DemoSection({ lang = "pl" }: { lang?: Lang }) {
  ...
  return (
    <section className="section-pad" id="przyklady">
      <div className="wrap">
        ...
        <p className="ex-note reveal">{copy.note}</p>
        <div className="examples">
          {PAIRS.map((pair, i) => (
            <div key={i} className="ex-card reveal">
              ...
              {copy.exampleLanguageNote ? (
                <p className="ex-lang-note">{copy.exampleLanguageNote}</p>
              ) : null}
